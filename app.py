...
# IMPORTANTE: Não importar streamlit aqui para evitar circular imports
# O streamlit será usado apenas na função adicionar_botao_pdf_empresa
# que é chamada a partir do código principal
...
def gerar_relatorio_empresa(df_filtrado, empresa, mes_selecionado=None, ano_selecionado=None):
    ...
    try:
        ...
        pdf.set_font('Arial', 'B', 12)
        pdf.set_text_color(0, 89, 115)
        pdf.cell(0, 8, '2. ACUMULADO DA UNIDADE', 0, 1, 'L')
        pdf.set_draw_color(2, 138, 159)
        pdf.line(10, pdf.get_y(), 200, pdf.get_y())
        pdf.ln(5)
        ...
        pdf.cell(0, 5, f'Equipamentos Comissionados: {comissionados_acumulado} ({pct_comiss_acumulado:.1f}%)', 0, 1)
        ...
        pdf.set_font('Arial', 'B', 12)
        pdf.set_text_color(0, 89, 115)
        pdf.cell(0, 8, '3. EQUIPE RESPONSÁVEL', 0, 1, 'L')
        pdf.set_draw_color(2, 138, 159)
        pdf.line(10, pdf.get_y(), 200, pdf.get_y())
        pdf.ln(10)
